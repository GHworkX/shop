...
import cn.xd.utils.PageHibernateCallback;
...
public class ProductDao extends HibernateDaoSupport {
	...
	// 根据seller的id查询商品的个数
	public int findCountSellerId(Integer sellerId) {
		String hql = "select count(*) from Product p where p.seller.uid = ?";
		List<Long> list = this.getHibernateTemplate().find(hql, sellerId);
		if (list != null && list.size() > 0) {
			return list.get(0).intValue();
		}
		return 0;
	...
	// 根据seller的id查询商品的集合
	public List<Product> findByPageSellerId(Integer sellerId, int begin, int limit) {
		String hql = "select p from Product p where p.seller.uid = ?";
		// 分页另一种写法:
		List<Product> list = this.getHibernateTemplate()
				.execute(new PageHibernateCallback<Product>(hql, new Object[] { sellerId }, begin, limit));
		if (list != null && list.size() > 0) {
			return list;
		}
		return null;
